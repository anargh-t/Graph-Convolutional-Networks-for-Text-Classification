"""
TextGCN Utility Functions

This module contains utility functions for the TextGCN implementation including:
...
These utilities support the main training and evaluation pipeline.
"""
...
def macro_f1(pred, targ, num_classes=None):
    ...
    Macro-averaging computes the metric for each class separately and then
    takes the average. This gives equal weight to all classes regardless of
    their frequency in the dataset.
    
    Args:
        pred (torch.Tensor): Model predictions (logits)
        targ (torch.Tensor): Ground truth labels
        num_classes (int, optional): Number of classes. If None, inferred from targets.
        
    Returns:
        tuple: (f1_score, precision, recall) - macro-averaged metrics
    """
    # Convert logits to predicted class indices
    pred = th.max(pred, 1)[1]
    
    # Initialize lists to store per-class metrics
    tp_out = []  # True positives for each class
    fp_out = []  # False positives for each class
    fn_out = []  # False negatives for each class
    ...
    for i in num_classes:
        # True positives: predicted as class i and actually class i
        tp = ((pred == i) & (targ == i)).sum().item()
        # False positives: predicted as class i but actually not class i
        fp = ((pred == i) & (targ != i)).sum().item()
        # False negatives: not predicted as class i but actually class i
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    values = th.from_numpy(sparse_mx.data).float()  # Ensure float32
